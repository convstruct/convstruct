...
class Core:
    ...
    def createActiveGraphs(self, ground, iicc_learning, struct, stage):
        ...
        graph['starting_points'] = multiPlaceholders(self.args['num_in'], 'starting_points', self.args['indir'])
        ...
        for gpu in range(self.specifications['gpus'] if stage == 2 else 1):
            ...
            with tf.device(("/gpu:%d" % gpu) if self.specifications['gpus'] >= (gpu + 1) and not self.specifications['multi_gpu_test'] else "/gpu:0"):
                graph_output['graph_output_%d' % gpu] = graph['generator_%d' % gpu].createGraph(graph['starting_points'], self.args['num_in'])
                graph['graph_real_%d' % gpu] = graph['discriminator_%d' % gpu].createGraph(ground, self.args['num_comp'], 1)
                graph['graph_fake_%d' % gpu] = graph['discriminator_%d' % gpu].createGraph(graph_output['graph_output_%d' % gpu], self.args['num_comp'], 1)
                if self.args['num_comp'] > 1:
                    grounded, generated = tf.concat(list(ground.values()), 3), tf.concat(graph_output['graph_output_%d' % gpu], 3)
                graph['sum_hat_%d' % gpu] = tf.random_uniform([], 0.0, 1.0) * (grounded if self.args['num_comp'] > 1 else ground) + (1 - tf.random_uniform([], 0.0, 1.0)) * (generated if self.args['num_comp'] > 1 else graph_output['graph_output_%d' % gpu])
                graph['sum_logits_%d' % gpu] = graph['discriminator_%d' % gpu].createGraph(graph['sum_hat_%d' % gpu], self.args['num_comp'], 1)
                graph['graph_loss_%d' % gpu] = graph['discriminator_%d' % gpu].createLoss(graph['graph_fake_%d' % gpu], graph['graph_real_%d' % gpu], graph['sum_hat_%d' % gpu], graph['sum_logits_%d' % gpu], self.getEarlyClassification(iicc_learning, ground, stage), self.getEarlyClassification(iicc_learning, graph_output['graph_output_%d' % gpu], stage))
                graph['graph_output_loss_%d' % gpu] = graph['generator_%d' % gpu].createLoss(graph['graph_fake_%d' % gpu])
                disc['discriminator_ops_%d' % gpu] = graph['discriminator_%d' % gpu].createOptimizer(graph['graph_loss_%d' % gpu])
                gen['generator_ops_%d' % gpu] = graph['generator_%d' % gpu].createOptimizer(graph['graph_output_loss_%d' % gpu])
        graph['gen'], graph['disc'] = gen, disc
        performanceTracker(start, 'Graph completed in %fs')
        return graph, graph_output, epoch_feed
    ...
    def getEarlyClassification(self, iicc_learning, content, stage):
        ...
        pool2_output = self.iicc.getFirstLayers(processed_content, iicc_learning, self.growth['x_size'] if stage != 2 else self.growth['small_x_size'])
        performanceTracker(start, 'IICC graph completed in %fs')
        return pool2_output
    ...
    def getModelEstimation(self, teacher, module_type, live_ops, struct, gpu):
        ...
        with tf.Session(config=tfConfigSetup()) as sess:
            ...
            performanceTracker(start, 'Module estimator completed in %fs')
            coord.request_stop()
            coord.join(threads)
    ...
    def createLatentModules(self, module_type, struct, gpu):
        ...
        struct['teacher_%d_split_%d' % (module_type, gpu)] = splitRandom(int(struct['teacher_%d_module_%d' % (module_type, gpu)]) // 3, int(np.random.uniform(self.growth['scaling'], int(struct['teacher_%d_module_%d' % (module_type, gpu)]) // 3, [1])[0]))
    ...
    def buildTopology(self, parameters, module_size, split, gpu, module_type):
        ...
        performanceTracker(start, '%s parameters completed in %fs', "Generator module %d" % gpu if module_type == 1 else "Discriminator module %d" % gpu)
    ...
    def prepareGraph(self, module_type, struct, stage, gpu):
        ...
        total_split, total_parameters = self.createLatentModules(module_type, struct, gpu)
        ...
        self.getModelTopology('generator' if module_type == 1 else 'discriminator', module_type, struct, gpu)
        ...
        if stage == 3 or stage == 4:
            ...
            _, _, live_ops = self.getModelEstimatorGraph(teacher, struct, stage)
            score = self.getModelEstimation(teacher, module_type, live_ops, struct, gpu)
        else:
            score = 1
        performanceTracker(start, 'Latent topology generator graph completed in %fs')
        ...
        for gpu in range(self.specifications['gpus'] if stage == 2 else 1):
            ...
            while True:
                if (total_ops > ((self.specifications['max_memory_%d' % gpu] * 0.75) // 2)) or (target_score > float(pred_score)) or module_type != 2:
                    ...
                    total_ops, struct, pred_score = self.prepareGraph(module_type, struct, stage, gpu)
                    ...
                    break
        tf.reset_default_graph()
        performanceTracker(start, 'Setting topologies completed in %fs')
    ...
    def getClassification(self, eval_feed, struct, stage):
        ...
        ground = multiPlaceholders(self.args['num_comp'], 'ground_truth_images')
        noise = multiPlaceholders(self.args['num_comp'], 'noise_images')
        augmented = multiPlaceholders(self.args['num_comp'], 'augmented_images')
        true_pair, fake_pair, noise_pair = self.iicc.getFullClassifier(ground, iicc_learning, self.growth['small_x_size']), self.iicc.getFullClassifier(augmented, iicc_learning, self.growth['small_x_size']), self.iicc.getFullClassifier(noise, iicc_learning, self.growth['small_x_size'])
        true_logits, fake_logits, noise_logits = [tf.argmax(self.iicc.getClassifierLogits(true_pair, 20, 'over' + str(i + 1)), axis=1) for i in range(5)], [tf.argmax(self.iicc.getClassifierLogits(fake_pair, 20, 'over' + str(i + 1)), axis=1) for i in range(5)], [tf.argmax(self.iicc.getClassifierLogits(noise_pair, 20, 'over' + str(i + 1)), axis=1) for i in range(5)]
        with tf.Session(config=tfConfigSetup()) as sess:
            start = time.time()
            ...
            performanceTracker(start, 'Saver completed in %fs')
            for gpu in range(self.specifications['gpus']):
                if not struct['nan_found']:
                    ...
                    main_feed = multiDictFeed(self.args, gpu_feed, augmented, eval_list if self.args['num_comp'] > 1 else eval_list[0], ground, eval_feed['comp_%d' % gpu], '2_end')
                    noise_feed = multiDictFeed(self.args, gpu_feed, noise, noise_list if self.args['num_comp'] > 1 else noise_list[0], ground, eval_feed['comp_%d' % gpu], '2_end')
                    start = time.time()
                    true_hat, fake_hat = sess.run([true_logits, fake_logits], main_feed)
                    noise_hat = sess.run(noise_logits, noise_feed)
                    performanceTracker(start, 'Reflection %d session completed in %fs', gpu)
                ...
                self.saveModelOutputs(eval_feed, gpu, stage)
                ...
                performanceTracker(start, 'Result %d session completed in %fs', gpu)
            return
    ...
    def startEstimator(self, stage):
        ...
        construct, loss, estimated = self.getModelEstimatorGraph(teacher, stage, quality)
        with tf.Session(config=tfConfigSetup()) as sess:
            coord, threads = initialization(sess)
            start = time.time()
            variables_to_restore = [var for var in tf.global_variables() if var.name.startswith('weights_') or var.name.startswith('bias_')]
            live = tf.train.Saver(variables_to_restore)
            performanceTracker(start, 'Saver completed in %fs')
            self.growth['model_target'] = calculateTarget(self.growth['model_quality'])
            ...
            for epoch in range(self.growth['estimator_length']):
                quality_mini_batch, teacher_mini_batch = setupMiniBatch(self.growth['teacher_feeds'], self.specifications)
                ...
                performanceTracker(start, 'Shaping session completed in %fs')
            ...
            performanceTracker(start, 'Saving discovery completed in %fs')
    ...
    def runGraphOps(self, sess, epoch, eval_feed, epochs_truth, struct, graph, models, epoch_feed, total_epochs, stage):
        ...
        if stage == 2 or stage == 3:
            for gpu in range(self.specifications['gpus'] if stage == 2 else 1):
                graph_loss, graph_output_loss = sess.run([graph['graph_loss_%d' % gpu], graph['graph_output_loss_%d' % gpu]], epoch_feed)
                if self.args['console']:
                    print("Epoch: %d " % epoch + "Discriminator %d model loss: " % (gpu + self.specifications['epoch_count'] + 1) + str(graph_loss), "Generator %d model loss: " % (gpu + self.specifications['epoch_count'] + 1) + str(graph_output_loss) + ")")
                if epoch != 0:
                    if math.isnan(graph_loss) or math.isnan(graph_output_loss):
            ...
            for gpu in range(self.specifications['gpus'] if stage == 2 else 1):
                eval_feed['output_%d' % gpu], eval_feed['comp_%d' % gpu] = generated_output['graph_output_%d' % gpu], epochs_truth
        if stage == 4:
            output = sess.run(models, epoch_feed)
            eval_feed['output_0'] = output['graph_output_0']
        performanceTracker(start, 'Session %d completed in %fs', epoch)
    ...
    def stage_save(self, epoch, sess, draw, learn, first_learned, stage):
        ...
        performanceTracker(start, 'Saving session %d completed in %fs', epoch)
        return
    ...
    def runStage(self, stage):
        ...
        createLog(self.location, 'draw.log' if stage == 3 or stage == 4 else ('live.log' if stage == 2 else 'learn.log'))
        struct, total_ops = self.getGraphs(stage) if stage == 2 or (stage == 3 and self.growth['saved_epoch'] == 0) else (dict() if stage == 1 else np.load(os.path.join(self.location, 'struct.npy')).flat[0], None)
        get_ground_truth_batch = buildInputs(self.args, self.args['compdir'], self.args['num_comp'], self.growth, stage)
        get_starting_point_batch = buildInputs(self.args, self.args['indir'], self.args['num_in'], self.growth, stage) if self.args['indir'] else None
        iicc_learning = tf.placeholder(tf.bool, name='iicc_learning')
        truth = multiPlaceholders(self.args['num_comp'], 'ground_truth')
        augmented = multiPlaceholders(self.args['num_comp'], 'augmented')
        graph, models, epoch_feed = self.createActiveGraphs(truth, iicc_learning, struct, stage) if stage != 1 else self.iicc.getGraph(augmented, truth, iicc_learning, self.growth)
        with tf.Session(config=tfConfigSetup()) as sess:
            coord, threads = initialization(sess)
            draw, learn, first_learned = self.getSaver(sess, stage)
            ...
            while True:
                if epoch < total_epochs:
                    epochs_ground_truth, epochs_starting_points, epochs_augmented = setupBatch(self.args, sess, get_ground_truth_batch, get_starting_point_batch, self.growth, epoch if stage == 1 else None)
                    ...
                    epoch_feed = multiDictFeed(self.args, epoch_feed, augmented if stage == 1 else graph['starting_points'], epochs_augmented if stage == 1 else epochs_starting_points, truth, epochs_ground_truth, stage)
                    struct['nan_found'], epoch = self.runGraphOps(sess, epoch, eval_feed, epochs_ground_truth, struct, graph, models, epoch_feed, total_epochs, stage)
                    if struct['nan_found']:
                        break
        ...
        performanceTracker(main_start, ('Live loop completed in %fs' if stage != 3 else 'Draw completed in %fs') if stage != 1 else 'Learn completed in %fs')
        return
